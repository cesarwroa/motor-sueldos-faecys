...
# Esto es vital para que tu HTML pueda hablar con el servidor
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  
...
def calcular_sueldo(d: DatosEmpleado):
    # 1. Buscar Escala en el archivo maestro (escalas.py)
    escala = escalas.buscar_escala(d.rama, d.agrup, d.categoria, d.mes)
    
    # Valores base (si no encuentra escala, usa lo que mande el front o 0)
    basico_escala = float(escala.get("Basico") or 0) if escala else (d.basico or 0)
    nr_escala = float(escala.get("No Remunerativo") or 0) if escala else (d.nrVar or 0)
    sf_escala = float(escala.get("Suma Fija") or 0) if escala else (d.nrSF or 0)
    ...
    # 4. Zona
    zona_rem = (basico_calc + antig_rem) * (d.zona / 100) # Zona sobre básico+antigüedad
    ...
    # Solo si la jornada es reducida y NO es Call Center (que tiene reglas propias)
    if prop < 1 and prop > 0 and "CALL CENTER" not in rama_u: 
