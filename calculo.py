...
def calcular_recibo(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if sind_solid: _add(items, "Art 100", ded=sind_solid, base=base_ap)
    if afil_pct: _add(items, "Sindicato Afiliación", ded=afil_pct, base=base_ap)
...
def _calcular_final(p: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if sind_solid:
        _add(items, 'Art 100', ded=sind_solid, base=base_ap)
    if afil_pct:
        _add(items, 'Sindicato Afiliación', ded=afil_pct, base=base_ap)
    if afil_fijo:
